...
function Todo({ todo, toggleComplete, deleteTodo }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const todoRef = doc(db, 'todos', todo.id); // Correctly referencing the document in Firestore
      await updateDoc(todoRef, { text: editedText }); // Update Firestore with new text
      setEditMode(false); // Exit edit mode
    } catch (error) {
      console.error('Error updating document: ', error); // Handle error if any
    }
  };
